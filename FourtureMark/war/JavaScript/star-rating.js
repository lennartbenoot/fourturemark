...
// doesn't work: ratings.cleanWhitespace();
	ratings.each(function(i) {
		...
		// innerHTML was a convenience property introduced by Microsoft as a
		// means of reading and writing the HTML content of an element
		// a) innerHTML is at least 200% slower on all 6 browsers tested
		// (Firefox, Netscape 6+, Internet Explorer 6+, Opera 7+, Safari 2,
		// Seamonkey 1) than modifying the nodeValue and
		// b) innerHTML is NOT a World Wide Web Consortium web standard DOM 1
		// attribute.
...
function displayHover(ratingId, star) {
	$R(1,star).each(function(i) {
		$('star_'+ratingId+'_'+i).setAttribute('src', 'images/rating_over.gif');
	});
}
...
function displayNormal(ratingId, star) {
	$R(1,star).each(function(i) {	
		...
		$('star_'+ratingId+'_'+i).setAttribute('src', 'images/rating_'+status+'.gif');
	});
}
